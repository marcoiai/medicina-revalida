...
class QuestionScopeTest extends TestCase
{
    ...
    public function test_questions_endpoint_can_filter_by_ai_generated_comments(): void
    {
        $source = QuestionSource::create([
            'name' => 'Revalida INEP 2024 - Objetiva',
            'institution' => 'INEP',
            'exam' => 'Revalida',
            'url' => 'https://download.inep.gov.br/revalida/provas_e_gabaritos/2024_1_PV_objetiva_regular.pdf',
        ...
        $this->makeQuestion($source->id, null, 'revalida', 'ai');
        $this->makeQuestion($source->id, null, 'revalida', 'official');

        $response = $this->getJson('/api/questions?comment_source=ai');

        $response->assertOk();
        $response->assertJsonCount(1, 'data');
        $response->assertJsonPath('total', 1);
        $response->assertJsonPath('data.0.comment_source', 'ai');
    }

    public function test_questions_endpoint_can_filter_by_non_ai_comments(): void
    {
        $source = QuestionSource::create([
            'name' => 'Revalida INEP 2024 - Objetiva',
            'institution' => 'INEP',
        ...
        $this->makeQuestion($source->id, null, 'revalida', 'ai');
        $this->makeQuestion($source->id, null, 'revalida', 'official');
        $this->makeQuestion($source->id, null, 'revalida', null);

        $response = $this->getJson('/api/questions?comment_source=not_ai');
