...
class RevalidaGenerateCommand extends Command
{
    protected $signature = 'revalida:generate
        ...
        {--with-ai : Executa o gerador de IA após criar os prompts}';
    ...
    public function handle(): int
    {
        ...
        $this->line("Próximo passo: php artisan revalida:review --run={$runId}");
        if ($aiEnabled) {
            $this->line("Se quiser gerar via IA no mesmo fluxo: php artisan revalida:generate --run-id={$runId} --amount={$amount} --area={$areaFilter} --batch-size={$batchSize} --with-ai");
        } else {
...
PROMPT;
    ...
    private function runAiGeneration(string $promptsDir, string $generatedPath, string $rejectedPath): int
    {
        ...
        if (!$process->isSuccessful()) {
            $this->error('Falha na geração automática via IA.');
            return self::FAILURE;
        }
