...
AI_PROVIDER = normalize_provider_name(os.getenv("QUESTION_AI_PROVIDER", "openai"))
AI_MODEL = (
    os.getenv("OPENAI_MODEL")
    if AI_PROVIDER == "openai"
    else os.getenv("GEMINI_MODEL")
) or default_model_for_provider(AI_PROVIDER)
MAX_CONCURRENT = int(os.getenv("MAX_CONCURRENT_BATCHES", "6"))
QUESTIONS_PER_BATCH = int(os.getenv("QUESTIONS_PER_BATCH", "25"))
MAX_RETRIES = int(os.getenv("AI_RETRY_ATTEMPTS", os.getenv("GEMINI_RETRY_ATTEMPTS", "5")))
INITIAL_RETRY_DELAY_SECONDS = float(os.getenv("AI_RETRY_INITIAL_DELAY_SECONDS", os.getenv("GEMINI_RETRY_INITIAL_DELAY_SECONDS", "2")))
MAX_RETRY_DELAY_SECONDS = float(os.getenv("AI_RETRY_MAX_DELAY_SECONDS", os.getenv("GEMINI_RETRY_MAX_DELAY_SECONDS", "30")))
REQUEST_TIMEOUT_SECONDS = float(os.getenv("AI_REQUEST_TIMEOUT_SECONDS", os.getenv("GEMINI_REQUEST_TIMEOUT_SECONDS", "240")))
...
async def generate_with_retry(final_prompt: str, file_path: Path, sem: asyncio.Semaphore):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            async with sem:
                if client is None:
                    raise RuntimeError("Cliente de IA não inicializado.")
...
async def generate_from_file(file_path: Path, sem: asyncio.Semaphore, source_meta: dict | None):
    ...
    try:
        response_text, attempts = await generate_with_retry(final_prompt, file_path, sem)
...
async def main():
    global AI_PROVIDER, AI_MODEL, client
    ...
    parser.add_argument("--provider", default=os.getenv("QUESTION_AI_PROVIDER", "openai"), help="Provedor de IA (openai ou gemini)")
    ...
    print(
        ...
        f"provider={provider}, modelo={AI_MODEL}, timeout_por_lote={REQUEST_TIMEOUT_SECONDS:.0f}s",
        flush=True,
    )
