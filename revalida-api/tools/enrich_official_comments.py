...
COMMENT_PROVIDER = normalize_provider_name(os.getenv("QUESTION_COMMENT_PROVIDER", os.getenv("QUESTION_AI_PROVIDER", "openai")))
COMMENT_MODEL = (
    os.getenv("QUESTION_COMMENT_MODEL")
    or (os.getenv("OPENAI_MODEL") if COMMENT_PROVIDER == "openai" else os.getenv("GEMINI_MODEL"))
    or default_model_for_provider(COMMENT_PROVIDER)
)
...
GENERIC_COMMENT_PATTERNS = (
    re.compile(r"^errad[oa]\b", re.IGNORECASE),
    re.compile(r"^gabarito(\s+definitivo)?(\s+oficial)?(\s+do\s+inep)?\b", re.IGNORECASE),
    re.compile(r"^padr[aã]o\s+de\s+resposta\b", re.IGNORECASE),
    re.compile(r"^resposta\s+oficial\b", re.IGNORECASE),
    re.compile(r"^cancelad[oa]\b", re.IGNORECASE),
)
...
def needs_comment_enrichment(item: dict) -> bool:
    ...
    if comment:
        if comment.startswith(GENERIC_COMMENT_PREFIX):
            return True
        ...
        if " gabarito " in f" {comment} " and len(comment.split()) <= 12:
            return True
...
async def enrich_comments(questions: list[dict], limit: int, include_all_missing: bool = False) -> tuple[int, int]:
    ...
    for batch_number, index_batch in enumerate(batches, start=1):
        ...
        for generated in generated_items:
            ...
            questions[target_index]["comment_source"] = "ai"
            updated += 1
...
def main() -> None:
    ...
    parser.add_argument("--provider", default=os.getenv("QUESTION_COMMENT_PROVIDER") or os.getenv("QUESTION_AI_PROVIDER", "openai"), help="Provedor de IA (openai ou gemini).")
    parser.add_argument("--all-missing", action="store_true", help="Enriquece qualquer questão sem comentário, não apenas as oficiais.")
    args = parser.parse_args()

    input_path = Path(args.input)
    output_path = Path(args.output) if args.output else input_path
    provider = normalize_provider_name(args.provider)
    global client, COMMENT_PROVIDER, COMMENT_MODEL
    COMMENT_PROVIDER = provider
    COMMENT_MODEL = (
        os.getenv("OPENAI_MODEL") if provider == "openai" else os.getenv("GEMINI_MODEL")
    ...
    attempted, updated = asyncio.run(enrich_comments(questions, limit=args.limit, include_all_missing=args.all_missing))
