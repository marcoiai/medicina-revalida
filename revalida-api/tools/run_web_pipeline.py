...
DEFAULT_SOURCES_PATH = TOOLS / "sources.json"
...
def main():
    ...
    parser.add_argument("--generate", action="store_true", help="Executa a geração automática via IA")
    ...
    parser.add_argument("--ai-provider", default=os.getenv("QUESTIONS_AI_PROVIDER", ""), help="Provedor de IA para fallback e revisão (openai ou gemini)")
    ...
    parser.add_argument(
        "--category",
        default=os.getenv("QUESTIONS_IMPORT_CATEGORY", "revalida"),
        help="Categoria de estudo para a importação final. Use revalida ou estudo_geral.",
    )
    parser.add_argument(
        "--sources-file",
        default=os.getenv("QUESTIONS_SOURCES_FILE", str(DEFAULT_SOURCES_PATH)),
        help="Arquivo JSON com fontes de crawl. Padrão: tools/sources.json",
    )
    args = parser.parse_args()
    environment = os.environ.copy()
    if args.ai_provider:
        environment["QUESTION_AI_PROVIDER"] = args.ai_provider
    ...
    if run_all or args.crawl:
        ...
        if args.sources_file:
            crawl_command.extend(["--sources", args.sources_file])
        if args.exam:
        ...
        run_step(crawl_command, env=environment)
    if run_all or args.download:
        run_step([PYTHON_BIN, "-u", str(TOOLS / "download_pdfs.py")], env=environment)
    if run_all or args.parse:
        run_step([PYTHON_BIN, "-u", str(TOOLS / "parse_pdfs.py")], env=environment)

    official_questions_count = 0
    if run_all or args.extract_official:
        run_step([PYTHON_BIN, "-u", str(TOOLS / "extract_official_questions.py")], env=environment)
    ...
    if run_all:
        if official_questions_count > 0:
            ...
            if not args.skip_ai:
                ...
                if args.ai_provider:
                    generate_command.extend(["--provider", args.ai_provider])
                run_step(generate_command, env=environment)
    else:
        if args.batch:
            batch_env = os.environ.copy()
            run_step([PYTHON_BIN, "-u", str(TOOLS / "make_ai_batches.py")], env=batch_env)
    ...
    if run_all:
        if official_questions_count > 0 or not args.skip_ai:
            import_category = str(args.category or "revalida")
            run_step(["php", "artisan", "questions:import-json", f"--category={import_category}"], env=environment)
        else:
            print(
